...
int main (void){
	...
	ptr_cosecha = &Arreglo_cosecha[0];
	while(ptr_cosecha < &Arreglo_cosecha[opc]){
		printf("\n Nombre del agricultor para la cosecha %d > ",(ptr_cosecha-&Arreglo_cosecha[0])+1);		
		gets(ptr_cosecha->nombre); //Terminador '\0'
		//fgets(ptr_cosecha->nombre,TAM,stdin); //Terminador '\n'
		fflush(stdin); //Liberar buffer
		ptr_cosecha+=1;
	}
	
	for(ptr_cosecha = &Arreglo_cosecha[0]; ptr_cosecha < &Arreglo_cosecha[opc] ; ptr_cosecha+=1){
		printf("\n\n\n *************************** Cosecha %d  -  Agricultor: %s ***************************",(ptr_cosecha-&Arreglo_cosecha[0])+1,ptr_cosecha->nombre);
